var tutorialLayer = cc.LayerColor.extend({
    ...
    init: function() {
        if (this.ls.getItem(666) == 2 || this.ls.getItem(666) == 3) { //because native jsb only allow init()
            this.initTwo();
        } else {
    ...
    initAnimations: function() {
        this.rightUpTut = cc.moveBy(0.1, cc.p(+this.sizeOfSprite, +this.sizeOfSprite));
    ...
    update: function(dt) {
        if (this.mode == 1) {
            ...
            if (this.counter == 2 && this.animations == 0) {
    ...
    changeRails: function(railPosY, railPosX) {
        var winSize = cc.director.getWinSize();
        var ls = cc.sys.localStorage;
        if (this.leswitcher.gameModeThree == 0) {
        ...
        } else if (this.leswitcher.gameModeThree == 1) {
            switch (this.level[railPosY][railPosX]) {
                case 1:
                case 3:
                    var spriteDel = this.tutLayer.getChildByTag(1000 * railPosY + railPosX);
